...
//   final Widget? child; // ضفنا الـ child هنا
//   const CustomCartoonButton({
//     super.key,
//     required this.text,
//     required this.color,
//     required this.onTap, required MaterialAccentColor backgroundColor, this.style, this.child,
//   });
//
//   @override
...
class CustomCartoonButton extends StatefulWidget {
  ...
  final Widget? child; // الـ child اللي ضفناه
  ...
  const CustomCartoonButton({
    ...
    required this.color,
    required this.onTap,
    required Color backgroundColor, // غيرتها لـ Color عشان تقبل أي لون مش بس Accent
    this.style,
    this.child,
  });
...
class _CustomCartoonButtonState extends State<CustomCartoonButton> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Stack(
        children: [
          // 1. جسم الزرار الـ 3D (الطبقة الأساسية)
          Container(
            width: double.infinity,
            // جعلنا الـ padding مرن عشان لو فيه child واخد مساحة
            ...
            child: Center(
              // ✨ التعديل هنا: لو فيه child اعرضه، لو مفيش اعرض النص الافتراضي
          ...
          // 2. طبقة اللمعة المتحركة (تأثير الـ Shine)
          Positioned.fill(
            child: ClipRRect(
              borderRadius: BorderRadius.circular(25.r),
              child: AnimatedBuilder(
